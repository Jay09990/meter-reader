...
export function PeriodSelector({ value, onChange }: { value: ConsumptionMode; onChange: (value: ConsumptionMode) => void }) {
  return (
    <div className="relative grid grid-cols-3 rounded-lg bg-secondary p-1 text-xs font-semibold">
      <span className="absolute inset-y-1 w-1/3 rounded-md bg-card shadow-sm transition-transform duration-200" style={{ transform: `translateX(${PERIODS.findIndex((period) => period.value === value) * 100}%)` }} />
      {PERIODS.map((period) => (
