...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getPaginatedCustomersWithDevices({
      page,
      limit,
      search,
      status,
    ...
    logApi("GET /api/customers/grouped → 200", { totalCount: result.pagination.totalCount, returned: result.items.length });
